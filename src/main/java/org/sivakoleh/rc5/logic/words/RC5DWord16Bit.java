...
// Uses short as a 16-bit word storage unit
public class RC5DWord16Bit implements IRC5DWord {
    private static final int WORD_BIT_COUNT = 16;
    ...
    RC5DWord16Bit(byte[] dataInBytes, int[] S, int roundsCount) {
        this.S = S;
        this.roundsCount = roundsCount;

        short[] data = new short[dataInBytes.length / 2];
    ...
    public byte[] performEncryptionRounds() {
        ...
        for (int i = 1; i <= roundsCount; ++i) {
            A = (short) (rotateLeft((A ^ B), B) + S[2 * i]);
            B = (short) (rotateLeft((B ^ A), A) + S[2 * i + 1]);
        }

        ByteBuffer resultBuffer = ByteBuffer.allocate(8);
        resultBuffer.putShort((short) A);
        resultBuffer.putShort((short) B);
